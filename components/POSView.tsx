  ...
  if (!activeShift) {
      return (
        <div className="h-full w-full flex flex-col items-center justify-center p-8 text-center animate-fade-in relative overflow-hidden">
            <div className="bg-white/70 backdrop-blur-xl p-12 rounded-[3rem] shadow-2xl shadow-indigo-100/50 border border-white max-w-md w-full relative z-10">
                <div className="w-24 h-24 bg-gradient-to-tr from-indigo-100 to-violet-100 rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-inner">
    ...
    <div className="h-full flex overflow-hidden">
        <div className="flex-1 flex flex-col p-6 overflow-hidden relative z-10">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4 bg-white/80 backdrop-blur-md p-2 pr-6 rounded-full shadow-sm border border-white">
                ...
                <button 
                    ...
                    className="px-6 py-3 bg-white/80 backdrop-blur-md border border-white text-slate-600 rounded-2xl font-bold text-xs flex items-center gap-2 shadow-sm hover:bg-white hover:shadow-md transition-all active:scale-95"
                >
            ...
            <div className="flex gap-4 mb-4">
                <div id="pos-view-toggles" className="flex gap-1 bg-white/80 backdrop-blur-md p-1.5 rounded-[1.5rem] border border-white h-[64px] items-center shrink-0 shadow-sm">
                ...
                <div id="pos-scanner-section" className="flex-1 relative group">
                    ...
                    <input 
                        ...
                        className="w-full h-[64px] pl-16 pr-4 bg-white/80 backdrop-blur-md border border-white rounded-[1.5rem] focus:border-indigo-300 focus:ring-4 focus:ring-indigo-50/50 outline-none font-bold text-lg text-slate-700 transition-all placeholder-slate-300 shadow-sm" 
            ...
            <div className="flex gap-4 mb-6 items-center overflow-x-auto pb-2 custom-scrollbar" id="pos-search-bar">
                <div className="w-64 relative shrink-0">
                    ...
                    <input 
                        ...
                        className="w-full pl-12 pr-4 py-3.5 bg-white/80 backdrop-blur-md border border-white rounded-2xl shadow-sm focus:ring-2 focus:ring-indigo-50/50 outline-none transition-all font-bold text-sm text-slate-700" 
                        value={searchTerm} 
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                </div>
                <div className="flex gap-2" id="pos-categories">
                    <button onClick={() => setSelectedCategory('Todos')} className={`px-6 py-3.5 rounded-2xl text-xs font-bold whitespace-nowrap transition-all border ${selectedCategory === 'Todos' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200 scale-105' : 'bg-white/80 border-white text-slate-500 hover:bg-white'}`}>Todos</button>
                    {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-3.5 rounded-2xl text-xs font-bold whitespace-nowrap transition-all border ${selectedCategory === cat ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200 scale-105' : 'bg-white/80 border-white text-slate-500 hover:bg-white'}`}>{cat}</button>
                    ))}
                </div>
            </div>
                        ...
                        {filteredProducts.map((p: Product, idx: number) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`
                                        bg-white p-5 rounded-[2rem] border transition-all duration-300 relative flex flex-col justify-between h-[220px] animate-fade-in-up
                                        ${isOutOfStock 
                                            ? 'opacity-60 grayscale cursor-not-allowed border-slate-100 bg-slate-50' 
                                            : `cursor-pointer group hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-200/40 active:scale-95 
                                               ${inCartQty > 0 ? 'border-indigo-500 ring-4 ring-indigo-50/50' : 'border-white shadow-sm'}`}
                                    `}
                                    style={{animationDelay: `${idx * 40}ms`}}
                                >
                                    {inCartQty > 0 && !isOutOfStock && (
                                        <div className="absolute top-4 left-4 z-10 bg-indigo-600 text-white w-8 h-8 flex items-center justify-center rounded-full text-xs font-black shadow-lg shadow-indigo-300 animate-bounce">
                                            {inCartQty}
                                        </div>
                                    )}
                                    ...
                                    <div className="flex justify-end absolute top-4 right-4 z-10">
                                        {isOutOfStock ? (
                                            ...
                                            <div className={`
                                                px-3 py-1.5 rounded-xl text-[11px] font-black flex items-center gap-1.5 border shadow-sm
                                                ${isLowStock 
                                    ...
                                    <div>
                                        ...
                                        <h3 className="font-bold text-slate-800 leading-tight mb-3 truncate text-sm">{p.name}</h3>
                                ...
                                {filteredProducts.map((p: Product) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="p-5">
                                                <span className={`text-[10px] px-3 py-1 rounded-lg font-black border ${p.stock <= 5 ? 'bg-rose-50 border-rose-100 text-rose-500' : 'bg-emerald-50 border-emerald-100 text-emerald-600'}`}>
                                                    {p.stock} UN
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-5 text-right">
                                                {!isOutOfStock && (
                                                    <button className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${inCartQty > 0 ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200 text-slate-400'}`}>
                ...
                <div className="absolute inset-0 z-50 bg-slate-900/20 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up border border-white">
                        <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsVariantModalOpen(false)} className="w-10 h-10 rounded-full bg-slate-50 hover:bg-rose-50 hover:text-rose-500 transition-colors flex items-center justify-center"><X className="w-5 h-5"/></button>
                        </div>
                        <div className="p-6 grid gap-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            {selectedProductForVariant.variants?.map((v) => (
                                <button
                                    ...
                                    disabled={v.stock <= 0}
                                    className={`p-5 rounded-[1.5rem] border text-left transition-all ${v.stock <= 0 ? 'opacity-60 bg-slate-50' : 'bg-white hover:shadow-lg hover:border-indigo-300'}`}
                                >
                                    <div className="flex justify-between">
                                        <span className="font-bold text-slate-800">{v.name}</span>
                                        <span className="font-black text-emerald-600">{v.stock} UN</span>
                                    </div>
                                    <p className="text-indigo-600 font-bold">{settings.currency}{v.price.toFixed(2)}</p>
                                </button>
                            ))}
                        </div>
                    </div>
        ...
        <div id="pos-cart" className="w-[400px] bg-white/90 backdrop-blur-xl border-l border-white shadow-2xl shadow-indigo-100 z-20"><Cart items={cart} onUpdateQuantity={onUpdateCart} onRemoveItem={onRemoveFromCart} onUpdateDiscount={onUpdateDiscount} onCheckout={onCheckout} onClearCart={onClearCart} settings={settings} customers={customers} /></div>
    </div>
  );
};